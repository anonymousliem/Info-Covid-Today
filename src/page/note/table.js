...
import {urlTugas, keteranganList} from '../../Constant';
...
class Tables extends Component {
  ...
  handleChange = selectedOption => {
    this.setState({ selectedOption : selectedOption, stateKeterangan : selectedOption.value});
    };
    ...
    axios({
      ...
      .then(data => {
        ...
        this.setState({
          ...
          "stateKeterangan" : data.data.status
        });
      })
      .catch(err => {
        console.log(err);
...
handlesubmitUpdate = () => {
  ...
  const Data = {
    ...
    "status" : this.state.stateKeterangan
  }
  axios({
    method: 'post',
    ...
    const renderresults = currentresults.map((results, index) => {
      ...
      return (
        <tr key={results._id} data-category={results._id}>
         ...
         <td>{results.date}</td>
         <td>{results.status == "done" ? <p style={{backgroundColor : "green", color:"black"}}>Done</p> : (
          results.status == "doing" ? <p style={{backgroundColor : "yellow", color:"black"}}>Doing</p> :
          <p style={{backgroundColor : "red", color:"black"}}>Do</p>
         )}</td>
         <td>
